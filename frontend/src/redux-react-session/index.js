    ...
    const parseCookies = (req) => {
      ...
      rc &&
        rc.split(";").forEach((cookie) => {
          ...
          if (
            parts[0].trim() === USER_SESSION ||
            parts[0].trim() === USER_DATA
          ) {
  ...
  static saveFromClient(cookies) {
    return new Promise((resolve, reject) => {
      if (cookies[USER_SESSION]) {
        sessionService.saveSession(cookies[USER_SESSION]).then(() => {
          if (cookies[USER_DATA]) {
            sessionService.saveUser(cookies[USER_DATA]).then(() => resolve());
          } else {
            return resolve();
          }
        });
